...
export function HeroSection({ withHeader = true }: { withHeader?: boolean }) {
  const t = useTranslations('hero');
  const tc = useTranslations('common');

  return (
    <>
      {withHeader ? <HeroHeader /> : null}
      ...
      <nav data-state={menuState && 'active'} className="fixed z-20 w-full px-2 group">
        <div
          className={cn(
            ...
            isScrolled && 'bg-black/60 max-w-4xl rounded-2xl border backdrop-blur-lg lg:px-5'
          )}
        >
